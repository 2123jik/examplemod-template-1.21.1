...
public class SpellDiscoveryUtil {
    ...
    public static void logAvailableSpells() {
        ...
        ExampleMod.LOGGER.info("=== Available Spells for Spell Trigger Affixes ===");
        ...
        spellsBySchool.forEach((school, schoolSpells) -> {
            ...
            schoolSpells.forEach(spell -> {
                ExampleMod.LOGGER.info("  - {} ({}): Level {}-{}, Cast Type: {}",
                    spell.getSpellName(),
                    spell.getSpellId(), 
                    spell.getMinLevel(), 
                    spell.getMaxLevel(),
        ...
        spells.stream()
                ...
                .forEach(spell -> {
                    ExampleMod.LOGGER.info("  - {} ({})", spell.getSpellName(), spell.getSpellId());
                });
    }
    ...
    public static List<AbstractSpell> getAvailableSpells() {
        ...
        spellClassNames.forEach(spellName -> {
            try {
                ...
                ExampleMod.LOGGER.error("SpellDiscovery error for {}: {}", spellName, e.getMessage());
            }
        });
